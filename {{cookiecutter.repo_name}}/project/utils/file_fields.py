...
def content_file_name(upload_to_path, instance, filename=None,
                      new_extension=None):
    """
    Méthode à utiliser pour l'attribut "upload_to" d'un FileField/ImageField
    pour renommer automatiquement le nom de fichier avec un nom unique et
    empêcher la conservation de fichier avec des caractères spéciaux.
    ...
    mais via un callable pour indiquer le chemin "upload_to" par exemple :
        ...
        ATTACH_FILE_UPLOADTO = lambda x,y: content_file_name(
            'attach/%Y/%m/%d', x, y)
...
def get_unique_filename(filename, new_filename=None, new_extension=None):
    ...
    (heures+minutes+secondes+microsecondes) soit un nouveau nom est spécifié et
    ...
    :param new_filename: (optional) Nouveau nom de fichier personnalisé, ceci
                         implique que la date ne sera pas insérée dans le
                         nouveau nom

    :type new_extension: string
    :param new_extension: (optional) Force une nouvelle extension de fichier au
                          lieu de celle de l'original. À spécifier sans le
                          "point" de début d'une extension.
    ...
    if not new_filename:
        ...
        return '%s%s%s%s.%s' % (unicode(now.hour).zfill(2),
                                unicode(now.minute).zfill(2),
                                unicode(now.second).zfill(2),
                                unicode(now.microsecond).zfill(6), extension)
    else:
...
def generate_valid_path(filename, path, create_it=True,
    ...
    if create_it:
        if not exists(absolute_path):
            ...
            raise IOError(
                "'%s' exists and is not a directory." % absolute_path)
